...
try {
    ...
    console.log('Starting Tomba Clearbit-Combined Actor...');
    ...
    if (input.emails && input.emails.length > 0) {
        ...
        for (const email of input.emails) {
            ...
            try {
                console.log(`Enriching company data for email: ${email}`);

                // Use Tomba's company enrichment method with rate limiting
                const tombaResult = await rateLimitedRequest(async () => enrichment.combined(email));
                ...
                if (tombaResult && tombaResult.data) {
                    ...
                    console.log(
                        `Found company data for: ${email} - ${tombaResult.data.organization || 'Unknown Company'}`,
                    );
                }
            } catch (error) {
